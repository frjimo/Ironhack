...
- (void)testAfterInsertingAPersonTheresSomeInTheDatabaseHavingCoffeInTheRain {
    ...
    NSFetchRequest *query = [[NSFetchRequest alloc]initWithEntityName:[Person description]];
    NSSortDescriptor *orderByName = [[NSSortDescriptor alloc]initWithKey:@"name" ascending:YES];
    
    [query setSortDescriptors:@[orderByName]];
    
    error = nil;
    NSArray<Person *> *result = [self.context  executeFetchRequest:query error:&error];
    
    XCTAssertNotNil(result);
    XCTAssertNil(error);
    XCTAssertTrue(result.count);
